...
class EpicKitchen(Dataset):
    ...
    def _check(self):
        ...
        for line in tqdm(self.meta_file):
            ...
            #     img = self._get_single_img(osp.join(self.root, name), idx)
            #     images.append(img)
    ...
    def _create_transform(self):
        ...
        # self.transform = create_data_augment(self.img_size, use_flip=True, random_crop=True, other=False)
    ...
    def _get_a_traj(self, traj_meta):
        ...
        # sample from clip (not work)
        # total_frames = traj_meta['end_idx'] - traj_meta['start_idx'] + 1
        # segment_size = total_frames // num_frames
        # remainder = total_frames % num_frames
        # sampled_numbers = []
        # for i in range(num_frames):
        #     start = traj_meta['start_idx'] + i * segment_size
        #     end = start + segment_size
        #     if i == num_frames - 1:  # 将剩余的帧放到最后一个 segment
        #         end += remainder
        #     selected_frame = random.randint(start, end - 1)
        #     sampled_numbers.append(selected_frame)
        
        # sample from all frames
        sampled_numbers = random.sample(range(traj_meta['start_idx'], traj_meta['end_idx'] + 1), num_frames)
        # start = traj_meta['start_idx']
        # end = traj_meta['end_idx']
        # total_frames = end - start + 1
        # # Choose one frame from the first 10%
        # first_frame = random.randint(start, start + int(0.1 * total_frames) - 1)
        # # Choose one frame from the last 10%
        # last_frame = random.randint(end - int(0.1 * total_frames) + 1, end)
        # middle_frames = random.sample(range(first_frame, last_frame), num_frames - 2)
        # sampled_numbers = [first_frame] + middle_frames + [last_frame]

        # sample consecutive frames
        # total_frames = traj_meta['end_idx'] - traj_meta['start_idx'] + 1 
        # start = np.random.randint(0, total_frames - num_frames + 1)
        # end = start + num_frames
        # sampled_numbers = list(range(traj_meta['start_idx'] + start, traj_meta['start_idx'] + end))
        ...
        '''
        change to read from memory
        '''
        # imgs_tmp = traj_meta['images']
        # frames = [
        #     imgs_tmp[i - traj_meta[['start_idx']]] for i in range(len(sorted_numbers))
        # ]
        ...
        # st_frame = torch.tensor(traj_meta['start_idx'])
        # ed_frame = torch.tensor(traj_meta['end_idx'])
        # f_label = [(i - traj_meta['start_idx']) / (traj_meta['end_idx'] - traj_meta['start_idx']) for i in sorted_numbers]
        ...
        # add noise to f_label, the number is [-(f_label[1] - f_label[0]) / 10, (f_label[1] - f_label[0]) / 10]
        # noise = (torch.rand(f_label.size()) - 0.5) * 2 * (f_label[1] - f_label[0]) / 10
        # f_label = f_label + noise
...
def train_one_epoch(model: torch.nn.Module, 
    ...
    metric_logger.add_meter('loss_rnc', utils.SmoothedValue(window_size=5, fmt='{value:.6f}'))
    ...
    for visual_input, num_frames, f_labels, text_input in metric_logger.log_every(data_loader, print_freq, header):
        ...
        rnc_model, diff_model = loss_model
        with torch.cuda.amp.autocast():
            ...
            # rnc_text = text_features.clone().detach()
            loss_rnc, loss_bb = rnc_model(visual_features, text_features, num_frames, f_labels)
            # loss_diff = diff_model(visual_features, text_features, num_frames, f_labels)
            # loss_diff = torch.zeros_like(loss_rnc)
            # loss_rnc = torch.zeros_like(loss_diff)
        
        loss = loss_bb + loss_rnc
        # loss = loss_rnc
        ...
        metric_logger.update(loss_rnc=loss_rnc.item())
